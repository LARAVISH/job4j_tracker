...
public class StartUITest {
    ...
    public void whenDFindByIDItem() {
        ...
        String id = String.valueOf(item.getId());
        Input in = new StubInput(new String[]{"0", id, "1"});
        List<UserAction> actions = List.of(new FindItemBeIdAction(out), new ExitAction(out));
        new StartUI(out).init(in, tracker, actions);
    ...
    public void whenFindByName() {
        ...
        List<UserAction> actions = List.of(new FindItemsByNameAction(out), new ExitAction(out));
        new StartUI(out).init(input, tracker, actions);
