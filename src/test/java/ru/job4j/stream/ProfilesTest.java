...
class ProfilesTest {
    ...
    void collect() {
        ...
        List<Profile> profileList = List.of
                (new Profile(new Address("Moscow", "Lenina", 20, 199)),
    ...
    @Test
    void collectDeleteDuplicate() {
        List<Address> expect = List.of
                (new Address("Moscow", "Lenina", 20, 199),
                        new Address("Novosibirsk", "Vtoray", 3, 888),
                        new Address("Tyla", "Mira", 68, 857));
        Profiles profiles = new Profiles();
        List<Profile> profileList = List.of
                (new Profile(new Address("Moscow", "Lenina", 20, 199)),
                        new Profile(new Address("Moscow", "Lenina", 20, 199)),
                        new Profile(new Address("Tyla", "Mira", 68, 857)),
                        new Profile(new Address("Tyla", "Mira", 68, 857)),
                        new Profile(new Address("Novosibirsk", "Vtoray", 3, 888)),
                        new Profile(new Address("Novosibirsk", "Vtoray", 3, 888)));
        List<Address> res = profiles.collect(profileList);
        MatcherAssert.assertThat(res, is(expect));
    }
